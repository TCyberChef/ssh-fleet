...
ssh-fleet is an MCP server that provides SSH access to a fleet of machines from Codex. Single Python process, all machines, connection pooling, persistent shells. Registered globally with `--scope user`.
...
# Run the server directly (for debugging - normally launched by Codex via stdio)
ssh-fleet
```
...
- **`server.py`** - MCP tool definitions using FastMCP. Each `@mcp_server.tool()` function is a tool Codex can call. Global `store` (MachineStore), `pool` (ConnectionPool), and `_guide_output_dir` are module-level singletons.
...
The `render_command` tool runs a command through the existing `ConnectionPool` and renders its real output as a PNG terminal screenshot (Catppuccin Mocha, macOS window chrome, drop shadow, colored prompt). Internally the renderer still produces SVG - it is written to a tempfile and converted to PNG via `rsvg-convert` (no SVG file is left in the guide output dir). Pure stdlib on the Python side - no `cairosvg`, no Playwright, no new Python deps. The pure renderer lives in `ssh_fleet/terminal_render.py` and is trivially unit-testable; the MCP tool in `server.py` wires it to the pool and writes the PNG to `_guide_output_dir` (configurable via `guide_output_dir` in `~/.ssh-fleet/config.yaml`, defaults to `~/.ssh-fleet/guides`). **External requirement:** `brew install librsvg` (same dependency as the GIF pipeline). ANSI handling reuses `_strip_ansi` from `ssh.py` and layers a supplementary regex for bare C1 controls (like `\x1b=` / `\x1b>` emitted by `systemctl status` under a pty) that the shared regex doesn't catch - the fix lives in the renderer, not in `ssh.py`, to keep `SudoShell`'s marker-parsing behavior untouched. **Tip:** for guides that highlight errors, pass `sudo=False` - the default sudo path merges stderr into stdout at the PTY level, so the red-stderr rendering only triggers on the non-sudo path.
